...
import { Add, Edit, Filter, TrashCan, Dashboard, DocumentAdd, PresentationFile } from "@carbon/icons-react"
import {
  Breadcrumb,
  BreadcrumbItem,
  Button,
...
import { HeaderContainer, MainContainer, PageContainer, PageTitle } from "../styled.components"
import { ButtonContainer, CustomOverflowMenu, FilterIconActive, FilterIconDefault } from "./styled.components"
...
function Project() {
  ...
  const openAddModal = () => {
    ...
    setErrors({})
    setModalKey((prev) => prev + 1)
    const newSLNo = projects.length > 0 ? Math.max(...projects.map((p) => Number.parseInt(p.SL_NO))) + 1 : 1
    const initialFormData = {
      SL_NO: String(newSLNo),
      STAFF_VP: "",
      DIRECTOR: "",
  ...
  const handleDownloadPresentation = async () => {
    ...
    try {
      ...
      console.log("[v0] Download presentation clicked")
    } catch (error) {
                                          ...
                                          itemText={
                                            ...
                                            >
                                              <Checkbox
                                                ...
                                                onChange={(event) =>
                                                  handleFilterChange(header.key, value, event.target.checked)
                                                }
                                                style={{ display: "block" }}
                                              />
                                            </div>
                            ...
                            <TableRow key={key} {...rowPropsWithoutKey}>
                              {row.cells.map((cell) => (
                                <TableCell key={cell.id}>
                                  ...
                                  ) : cell.info.header === "DEPLOYMENT_DATE" ? (
                                    convertDateFormate(cell.value)
                  ...
                  if (!checkmarkFields.includes(key) && index < 12) {
                    return (
                      <tr key={key}>
                        <td className="pb-2 pr-2 heading textAlighCenter">
                          <span className="HeadingLabel">
                            {capitalizeFirstLetterOfEachWord(key.toLocaleLowerCase().replace(/_/g, " "))}:
                          </span>
                        </td>
                        <td className="pb-2 detail detailPad">
                          <TextInput
